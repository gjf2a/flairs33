...
pub fn to_kernalized(img: &Image, levels: usize, num_kernels: usize) -> Vec<BitImage> {
    let mut kernalized = vec![binary_via_brief_patch(img, KERNEL_SIZE)];
    for _ in 0..levels {
        let mut iterated = Vec::new();
        for src in kernalized.iter() {
...
pub fn kernalized_distance(k1: &Vec<BitImage>, k2: &Vec<BitImage>) -> R64 {
